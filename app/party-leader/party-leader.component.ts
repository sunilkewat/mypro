...
@Component({
  selector: 'app-party-leader',
  templateUrl: './party-leader.component.html',
  styleUrls: ['./party-leader.component.css']
})
export class PartyLeaderComponent implements OnInit {
  urlm:any;
  photo:any;
  submitText="";
  leaderTempId=0;
  ...
  constructor(private getservicename: AppServiceService, private formBuilder: FormBuilder,private router: Router) {
   ...
   getLeaderDataList()
   {
    this.getservicename.getPartyLeaderdataList({headers: this.myheader}).subscribe(data=>{  
  ...
  onSubmit(data){ 
    ...
    this.getservicename.postPartyLeaderData(this.LeaderModel,{headers: this.myheader})    
    .subscribe(res=>{
      if(res["response"]=="1")
      {
        $('.msgagr').html("New Party Leader Created Successfully").fadeIn('slow').css({"color": "#1e8449 ", "padding": "6px", "border-radius": "2px" }); //also show a success message 
      ...
      {
        $('.msgagr').html("Party Leader Record Update Successfully").fadeIn('slow').css({"color": "#1e8449 ", "padding": "6px", "border-radius": "2px" }); //also show a success message 
        $('.msgagr').delay(2000).fadeOut('slow');
      setTimeout(function(){
        $("#modal-default").modal('hide');
      },2000)
   ...
   { 
    this.LeaderModel=data;        
    this.getservicename.DeletePartyLeaderData(this.LeaderModel.leaderId,{headers: this.myheader})
    .subscribe(res => { 
